...
class Rational_Meta_Box {
	...
	public function add_meta_boxes() {
		foreach ( $this->screens as $screen ) {
			add_meta_box(
				'deadline-date',
				__( 'Deadline Date', 'msnb_deadline_meta_box' ),
	...
	public function add_meta_box_callback( $post ) {
		wp_nonce_field( 'deadline_date_data', 'deadline_date_nonce' );
	...
	public function generate_fields( $post ) {
		...
		foreach ( $this->fields as $field ) {
			...
			$db_value = get_post_meta( $post->ID, 'deadline_date_' . $field['id'], true );
			switch ( $field['type'] ) {
				default:
					$input = sprintf(
	...
	public function save_post( $post_id ) {
		if ( ! isset( $_POST['deadline_date_nonce'] ) )
			return $post_id;

		$nonce = $_POST['deadline_date_nonce'];
		if ( !wp_verify_nonce( $nonce, 'deadline_date_data' ) )
			return $post_id;
		...
		foreach ( $this->fields as $field ) {
			if ( isset( $_POST[ $field['id'] ] ) ) {
				...
				update_post_meta( $post_id, 'deadline_date_' . $field['id'], $_POST[ $field['id'] ] );
			} else if ( $field['type'] === 'checkbox' ) {
				update_post_meta( $post_id, 'deadline_date_' . $field['id'], '0' );
			}
		}
	}
}
